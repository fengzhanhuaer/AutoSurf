...
| REQ-014 | 发布并更新正式实例 | 提交并推送 `main`；CI 通过；在线升级完成；部署 SHA、健康、入口进程和至少一项 PT 行为可验证 | 高 | `进行中` | 用户“发布并更新” |
...
- 当前计划步骤：TASK-009
- 当前门禁：发布门禁进行中
- 最近完成检查点：发布前正式容器健康；本地 `main` 与 `origin/main` 均为 `8fa02b2`；全量测试、`compileall` 与 `git diff --check` 已通过。
- 工作区状态：本轮修改仍未提交、未推送、未部署，无用户既有改动被覆盖。
- 下一步唯一动作：提交并推送 `main`，监控 CI 后调用在线升级 API。
...
| TASK-009 | 发布、在线升级和正式回归 | `进行中` | REQ-014 | Git、GitHub Actions、升级 API、正式 PT API | CI、SHA、健康、入口进程和受影响站点有界回归均通过 |
...
| TEST-008 | 本轮站点与统计修正 | REQ-007 至 REQ-013 / TASK-008 | 聚焦 pytest、全量 pytest、`compileall`、`git diff --check` | M-Team 仅刷新；PTTime、0ff、SunnyPT 正确；死亡站点排除；统计字段准确 | 聚焦 72 项、全量 115 项通过；1 个既有弃用警告；编译与差异检查通过 | `已通过` | pytest 与命令结果 |
| TEST-009 | 图片验证自动识别可行性 | REQ-012 / TASK-008 | 正式最近执行分类；`pip install --dry-run ddddocr`；当前适配器路径复核 | 明确字符 OCR、语义视觉和人机挑战边界，不提交未确认答案 | OpenCD 可采用本地 OCR 试验；TJUPT 需视觉语义模型；Cloudflare 保持人工浏览器处理；当前容器未安装 OCR 运行时 | `已通过` | 正式执行、依赖 dry-run、代码复核 |
| TEST-010 | 发布与正式实例验证 | REQ-014 / TASK-009 | GitHub Actions、`/api/v1/system/upgrade`、容器与 PT 管理 API | 推送 SHA 与正式 HEAD 一致，升级 complete、服务 healthy、行为生效 | 待执行 | `进行中` | 待补 |

### 5.2 未执行测试

未执行提交、推送、部署和真实用户凭据签到，因为本轮只收到代码适配指令，未获得发布授权。使用无用户凭据的真实 Chromium 验证了签名链路，不产生用户签到记录。
...
| REQ-009 | 0ff 动态历史 | UNIT-002、UNIT-003 | TASK-008 | 历史提取器、测试 | TEST-008 | 有界等待并提取日期与奖励 | `已通过` |
| REQ-010 | LemonHD、GTK 死亡站点清理 | UNIT-001、UNIT-003 | TASK-008 | 忽略目录、任务协调、测试 | TEST-008 | 不再发现，现有任务可停用且历史保留 | `已通过` |
| REQ-011 | SunnyPT 当前签到页 | UNIT-002、UNIT-003 | TASK-008 | SunnyPT 适配器、目录、测试 | TEST-008 | 点击后必须确认今日状态，401 准确分类 | `已通过` |
| REQ-012 | 图片验证方案 | 当前架构 | TASK-008 | OpenCD/TJUPT 适配器、依赖评估 | TEST-009 | 三类验证已分流，未启用无证据自动提交 | `已完成` |
| REQ-013 | 统计纠错补全 | UNIT-002、IF-003 | TASK-008 | 专用 API 适配器、通用 DOM 提取、清洗器 | TEST-008 | 真实字段样本与接口形状测试通过 | `已通过` |
...
| DEC-006 | 0ff 日历由脚本动态渲染 | 提取前有界等待事件 DOM，并兼容事件嵌套和几何映射 | 当前立即抓取可能早于日历完成 | 写死日期或奖励 | 历史提取器 | 无 | `有效` |
...
| DEC-008 | 多个站点统计为空或错位 | Rousi、SunnyPT、Zhuque 优先用同源 API；其他站点增强结构化 DOM 提取与清洗 | API 字段稳定且可白名单化，DOM 卡片与导航混排会错配 | 继续只依赖相邻表格单元格 | 资料刷新与统计快照 | 无 | `有效` |
...
| RISK-002 | 行为风险 | 真实签到可能产生站点记录或奖励 | 回归具有外部副作用 | TTG、SunnyPT 各触发一次，未重复 | `已关闭` |
...
| RISK-006 | 正式凭据状态 | SunnyPT 正式 AutoSurf Cookie 当前仍跳转登录页，用户浏览器截图中的新会话尚未同步 | 新 API 统计代码无法立即生成正式快照 | 用户下次同步 Cookie 后再执行一次资料刷新；代码准确返回登录失效 | `外部待同步` |
...
| 检查项 | 结论 | 证据或条件 |
|---|---|---|
| 用户最新目标和有效需求逐项验收 | 通过 | REQ-007 至 REQ-013、TEST-008、TEST-009 |
| 端到端追踪闭合 | 通过 | 第六章 |
...
| 账本与工作区一致，下一步唯一动作为“无” | 通过 | 代码适配完成；未授权的发布动作明确排除 |

- 门禁结论：通过
- 条件及关闭要求：无；发布和真实用户签到不属于本轮授权范围。
...
- 交付结果：M-Team 改为只刷新资料；PTTime 403 已签到、0ff 动态历史、SunnyPT 当前签到页和死亡站点均已修正；Rousi、SunnyPT、Zhuque 使用专用同源 API，TJUPT 与 Audiences 统计解析已纠错。
- 验证方案：OpenCD 传统字符码适合本地 `ddddocr` 样本基准；TJUPT 为影视语义选择，需视觉模型或人工确认；Cloudflare 不自动破解。本轮未启用任何无置信度证据的自动提交。
- 需求验收：REQ-007 至 REQ-013 均完成；代码保留为未提交工作区修改，未发布。
- 测试结论：聚焦 72 项、全量 115 项通过；`compileall` 与 `git diff --check` 通过。
- 遗留外部条件：SunnyPT 正式 AutoSurf Cookie 仍需重新同步；部署后才会产生修正后的正式统计快照。
- 回滚说明：见第九章。
- 完成门禁：通过。
- 下一步唯一动作：无；提交、推送、在线升级和真实用户签到均未执行。
