...
| REQ-017 | 发布并验证本轮全部修复 | 聚焦及全量测试通过；提交推送并在线升级；每个受影响站点只触发一次有界回归 | 高 | `待开始` | 用户最新指令与发布约束 |
...
| TASK-013 | 提交、推送、在线升级和逐站有界回归 | `进行中` | REQ-017 | Git、升级 API、正式任务 API | SHA、健康和每个受影响站点的最新结果可验证 |
...
| REQ-017 | 修复发布并逐站有界验证 | 发布架构 | TASK-012、TASK-013 | Git、升级 API、正式任务 API | TEST-013、TEST-014 | 待实现与发布 | `待开始` |
...
| 检查项 | 结论 | 证据或条件 |
|---|---|---|
| 用户最新目标和有效需求逐项验收 | 通过 | REQ-007 至 REQ-014、TEST-008 至 TEST-010 |
| 端到端追踪闭合 | 通过 | 第六章 |
...
| 账本与工作区一致，下一步唯一动作为“无” | 通过 | 代码、测试、发布、在线升级和正式回归均完成 |

- 门禁结论：不通过
- 条件及关闭要求：REQ-015 至 REQ-017 尚未验收；完成矩阵、修复、测试、发布和有界回归后重新检查。
...
- 回滚说明：见第九章。
- 完成门禁：历史周期已通过；本轮 REQ-015 至 REQ-017 为不通过。
- 当前扩展周期：`进行中`；完成摘要以上内容仍是历史已交付结果，本轮以 REQ-015 至 REQ-017 为准。
- 下一步唯一动作：提交并推送当前差异，等待 CI 后通过 Web 在线升级。
