...
- 下一步唯一动作：HomePc 恢复可达后在线升级到最新 `main`，并只复用 OpenCD 当前执行一次。
...
| RISK-012 | 正式实例不可达 | 首轮五站有界回归后 HomePc 的 Web、SSH 和 ICMP 同时超时，无法继续调用在线升级或读取执行状态 | `fb5c485` 尚未部署，OpenCD 最终现场验收未完成 | 不重启、不直接写部署目录；等待主机/虚拟网关恢复后按远端精确 SHA 门禁在线升级 | `阻塞` |
...
- 当前扩展周期：`阻塞`；REQ-020 至 REQ-022 已有 `c33a2da` 正式证据，REQ-019 最终补丁 `fb5c485` 等待部署。
- 下一步唯一动作：HomePc 恢复可达后在线升级到最新 `main`，并只复用 OpenCD 当前执行一次。
