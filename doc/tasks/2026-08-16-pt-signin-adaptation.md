...
| REQ-025 | UBits 瞬态自动验证不应误报签到被拦截 | 首页自动验证在有界稳定复查中消失时继续访问签到页；验证持续存在或变为源站错误时保持真实失败分类；发布、在线升级并以当前执行做一次有界验证 | 高 | `进行中` | 用户“继续修复”“验证后发布升级” |
...
- 当前门禁：代码、聚焦测试、全量测试和编译通过；待推送、在线升级和 UBits 有界验证
...
| TASK-025 | 运行回归、发布、在线升级并有界验证 UBits | `进行中` | REQ-025 | pytest、Git、升级 API、PT 执行 API | 全量 203 项通过；待部署 SHA 一致和 UBits 单次验证 |
...
| `src/autosurf/automations/pt_signin.py`、`tests/test_pt_signin.py` | 首页自动验证结果增加一次有界稳定性复查 | UBits 在验证刚放行时截图已恢复正常，但首页分类仍保留先前验证文案 | REQ-025 / TASK-024 | TEST-023 | 第九章 |
...
| TEST-023 | UBits 瞬态自动验证稳定性 | REQ-025 / TASK-024、TASK-025 | `tests/test_pt_signin.py`、全量 pytest、在线升级后读取当前 UBits 执行 | 验证消失后继续签到；持续验证和雷池保持原有分类；部署后不重复创建活动执行 | 单元正反例和雷池无二次确认通过；聚焦 111 项、全量 203 项通过；线上待执行 | `进行中` | pytest、管理 API |
...
| REQ-025 | UBits 瞬态自动验证不应误报签到被拦截 | UNIT-011 | TASK-024、TASK-025 | `pt_signin.py`、测试、PT 执行 API | TEST-023 | 待实现、测试、发布和线上有界验证 | `进行中` |
...
| DEC-014 | UBits 验证页可能在分类与截图之间自动放行 | 对非雷池的首页自动验证只做一次 15 秒稳定性复查；确认验证消失后重新分类并继续签到页；雷池不重复点击确认 | 当前第 4 次 UBits 执行的失败截图为正常首页，实机同站可交替出现正常页和 Cloudflare Ray ID 页 | 立即把所有首页拦截视为可通过，或延长所有站点等待 | PT 首页分类 | 无 | `有效` |
...
| DEF-030 | REQ-025 / TEST-023 | 高 | UBits 首页的自动验证可能在 `_goto_pt_page` 等待结束后才放行；首页分类保留验证文本并提前返回，随后失败截图已恢复正常页面 | `代码已完成` | `pt_signin.py`、测试 | 聚焦 111 项、全量 203 项和编译通过；待线上证据 |
