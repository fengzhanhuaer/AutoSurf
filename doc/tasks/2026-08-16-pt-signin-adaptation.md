...
- 更新时间：`2026-08-19 23:00 +08:00`
- 用户原始需求：移除已经死亡的 PTLover，并结合今天签到与统计情况继续适配，尤其处理一次都没成功的站点。
- 用户最新指令：修正 U2 被记录为“已签到”但实际没有签到的问题；将 HomePc SSH 与 AutoSurf 网站调试拆成两个无必然依赖的技能，后续直接连接网站调试。
- 启用方式：明确长任务条件。
...
| REQ-015 | 建立当前全部签到异常矩阵 | 57 个 PT 任务和 1 个周期任务均有当前状态、最新结果、异常族、证据与处置结论；不沿用陈旧结果 | 高 | `进行中` | 用户“连接部署库，修复所有的签到异常” |
| REQ-016 | 修复全部代码可控的签到异常 | 每个确认代码缺陷都有根因、最小修复和回归测试；外部阻塞保持准确分类且不伪报成功 | 高 | `待开始` | 用户最新指令 |
| REQ-017 | 发布并验证本轮全部修复 | 聚焦及全量测试通过；提交推送并在线升级；每个受影响站点只触发一次有界回归 | 高 | `进行中` | 用户最新指令与发布约束 |
| REQ-018 | 修正 U2 虚假“已签到”结果 | U2 只有出现站点明确的今日签到证据或实际提交成功时才能返回 `already_done`/`success`；仅访问首页不得误报 | 高 | `进行中` | 用户“U2 没有签到” |
...
- 当前阶段：U2 修复发布
- 当前计划步骤：TASK-013 在线升级和 OpenCD/U2 逐站有界回归
- 当前门禁：准备门禁通过；完成门禁已重开
- 最近完成检查点：2026-08-20 `5fb8bbf` 已在线升级并完成 OpenCD 单次回归；U2 首页误判已修复，聚焦 93 项、全量 154 项通过。
- 工作区状态：`main` 与 `origin/main` 为 `ed67365`，启用本轮前工作区干净；当前仅本账本将产生任务内修改。
- 下一步唯一动作：提交推送 U2 修复，CI 通过后在线升级并只触发一次 U2 最终回归。
...
| TASK-013 | 提交、推送、在线升级和逐站有界回归 | `进行中` | REQ-017、REQ-018 | Git、升级 API、正式任务 API | SHA、健康和 OpenCD/U2 的最新结果可验证 |
...
| TEST-014 | 发布和受影响站点实测 | REQ-017 / TASK-013 | 在线升级 API、健康检查、每站单次 `run` 与有界轮询 | 代码版本一致；成功/已签到或准确外部阻塞；不重复触发 | `5fb8bbf` 在线升级完成且 SHA/健康一致；OpenCD 单次执行 `9709e365` 点击入口后返回 `already_done`，个人信息刷新成功 | `已通过` | 正式 API、CI 32274400785 |
| TEST-015 | U2 签到结果真实性 | REQ-018 / TASK-013、TASK-014 | 正式单次执行、执行详情、必要截图与聚焦测试 | 页面明确显示今日已签到或实际提交成功；仅首页访问不得成功 | 旧版单次执行 `0723ca19` 首页导航 60 秒超时并安全进入 `retry_wait`；新增首页误判反例，聚焦 93 项通过；最终正式复测待发布后执行 | `进行中` | 正式 API、pytest |
...
| REQ-017 | 修复发布并逐站有界验证 | 发布架构 | TASK-012、TASK-013 | Git、升级 API、正式任务 API | TEST-013、TEST-014、TEST-015 | `5fb8bbf` 与 OpenCD 已验证；U2 修复待发布和单次实测 | `进行中` |
| REQ-018 | U2 仅凭权威签到页返回成功 | UNIT-003 | TASK-013、TASK-014 | `pt_signin.py` | TEST-013、TEST-015 | 首页误判代码与测试已完成；待正式发布实测 | `进行中` |
...
| FACT-019 | 正式最近 200 条执行中，图片验证仅有 OpenCD 传统图片码与 TJUPT 图片选影视名称；Hdkyl 是 Cloudflare 人机验证 | 正式管理 API 只读查询 | 两类验证码需采用不同方案，Cloudflare 不进入图片 OCR |
...
| RISK-009 | U2 结果真实性 | 最近执行以首页文本返回 `already_done`，但无点击、无签到历史，且用户确认实际未签到 | 历史统计会把未签到误算为成功 | 现场单次执行取证；收紧 U2 成功证据并增加回归测试 | `处理中` |
...
| 必要架构和单元设计达到可实现程度 | 通过 | FACT-004 至 FACT-013 |
...
- 门禁结论：通过
- TASK-010 已完成当前 58 个任务矩阵；只对 DEF-018 至 DEF-021 进入实现，外部阻塞保持准确分类。

### 12.2 完成门禁

| 检查项 | 结论 | 证据或条件 |
|---|---|---|
| 用户最新目标和有效需求逐项验收 | 不通过 | REQ-018 / TEST-015 尚待正式发布实测 |
| 端到端追踪闭合 | 不通过 | U2 修复已测试但尚未在线升级 |
...
| 账本与工作区一致，下一步唯一动作为“无” | 不通过 | 仍需发布并单次回归 U2 |

- 门禁结论：不通过
- 条件及关闭要求：REQ-015 至 REQ-017 的原发布阻塞已关闭；REQ-018 需完成 U2 修复发布与单站有界回归。
...
- 回滚说明：见第九章。
- 完成门禁：历史周期已通过；本轮 REQ-018 尚未通过正式发布实测。
- 当前扩展周期：`进行中`；完成摘要以上内容仍是历史已交付结果，本轮以 REQ-015 至 REQ-018 为准。
- 下一步唯一动作：提交推送 U2 修复，CI 通过后在线升级并只触发一次 U2 最终回归。
